...
ListaComando : Comando  {$$ = $1;}
             | Comando ListaComando {$$ = mknode($1->left,$2,"Comando");}
             ;

Comando : 
	SEMI    {$$ = mknode(NULL, NULL, "SEMI");}
        | Expr SEMI     {$$ = mknode($1, NULL, "SEMI");}
        | RETORNE Expr SEMI     {$$ = mknode($2->left, $2->right, "Expr");}
        | LEIA LValueExpr SEMI  {$$ = mknode($2->left, $2->right, "Expr");}
        | ESCREVA Expr SEMI     {$$ = mknode($2->left, $2->right, "Expr");}
        | ESCREVA STRING SEMI   {$$ = mknode($1, $3, "STRING");}
        | NOVALINHA SEMI        {$$ = mknode(NULL, $2, "NOVALINHA");}
        | SE EPAREN Expr DPAREN ENTAO Comando {$$ = mknode($3->left,$3->right,"Expr");}
        | SE EPAREN Expr DPAREN ENTAO Comando SENAO Comando {$$ = mknode($3->left,$3->right,"Expr");}
        | ENQUANTO EPAREN Expr DPAREN EXECUTE Comando {$$ = mknode($3->left,$3->right,"Expr");}
        | Bloco         {$$ = $1;}
        ;
